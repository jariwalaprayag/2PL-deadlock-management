...
Patel Tejas: 1001729131
Jariwala Prayag: 1001719373
"""
...
def doit():
    global timestamp
    f = open(filename, 'r')
    #this3 for loop reads all the lines of inputfile one-time
    for x in f:
        ...
        if x[0]=='r':
            ...
            if transstate == 'aborted':
                ...
                locktid=str(st[3])
                if locktid=='':
                    db.locktable.update_one({'itemname':dataitem},{"$set":{'lockholdingtid':'-'+x[1] , 'lockstate':'read'}  })
                    ...
                    if count1>0:
                        ...
                        if state=='write':
                            ...
                            if int(wantingtidtimestamp)>int(holdingtidtimestamp):
                                ...
                                db.transactiontable.update_one({'tid': x[1]},{"$set": {'transstate': 'waiting'}    })
                                    ...
                                    if itemshold[i]!='-' and itemshold[i]!=dataitem:
                                        ...
                                        if state=='write':
                                            ...
                                            if len(lockwaitingtid)>0:
                                                ...
                                                st= str(cur[0]).split("'")
                                                tempo = str(st[3])
                                                cur1 = db.transactiontable.find({"tid": lockwaitingtid[2]}, {'transstate':1, '_id':0})
                                                st1= str(cur1[0]).split("'")
                                                if (str(st1[3]) != 'aborted'):
                ...
                #appropriate status(read,write) and transaction table is updated to show the dataitem in tid's held resources
                if count1==0:
                	db.locktable.insert({'itemname':dataitem,'lockstate':'read','lockholdingtid':'','lockwaitingtid':'-r'+x[1]})
                elif count1 > 0:
                	cur = db.locktable.find({'itemname': dataitem}, {'lockwaitingtid':1, '_id':0})
                	st= str(cur[0]).split("'")
...
#condition for writing instructions,executes only if transactive
            ...
            if transstate == 'aborted':
                ...
                if locktid=='':
                    ...
                    if count1>0:
                        ...
                        if int(wantingtidtimestamp)==int(holdingtidtimestamp):
                                if len(holdingtid) > 2:
                                    for i in range(holdingtid.index(wantingtid) + 2,len(holdingtid),2):
                                        db.transactiontable.update_one({'tid': holdingtid[i]}, {"$set":{'transstate':'aborted'} })
                                        # lock release code for every transaction
                                        cur = db.transactiontable.find({'tid': holdingtid[i]}, {'itemshold':1, '_id':0})
                                        st= str(cur[0]).split("'")
                                        itemshold=str(st[3])
                                        ######
                                        #condition for distributing aborted trans resources other than requested by requesting transaction
                                                    ...
                                                    if state=='read':
                                                        if len(lockholdingtid)>1:
                                                            ...
                                                            if len(lockwaitingtid)>0:
                                                                ...
                                                                if (str(st1[3]) != 'aborted'):
                                                                    db.transactiontable.update_one({"tid": lockwaitingtid[2]},{"$set": {"transstate": 'active',"itemshold":tempo+'-'+itemshold[i]}    })
                                db.locktable.update_one({"itemname": dataitem},{"$set": {"lockholdingtid": '-'+holdingtid[1],"lockstate": 'write'}    })
                        #if lock-holding tid is older than requesting tid
                        #then inlock table-requesting tid is added in lockwaitingtids column with status
                        #requesting,example -w3
                        if int(wantingtidtimestamp)>int(holdingtidtimestamp):
                            cur123 = db.locktable.find({'itemname': dataitem}, {'lockwaitingtid':1, '_id':0})
                            if state=='read':     #made changes
                                if len(holdingtid) > 2:
                                    for i in range(holdingtid.index(wantingtid) + 2,len(holdingtid),2):
                                        cur = db.transactiontable.find({'tid': holdingtid[i]}, {'timestamp':1, '_id':0})
                                        st= str(cur[0]).split(":")
                                        c=str(st[1])
                                        if c[2]=='}':
                                            holdingtidtimestamp=c[1]
                                            ...
                                            holdingtidtimestamp=c[1:3]
                                        if wantingtidtimestamp < holdingtidtimestamp:
                                            db.transactiontable.update_one({'tid': holdingtid[i]}, {"$set":{'transstate':'aborted'} })
                                              # lock release code for every transaction
                                            cur = db.transactiontable.find({'tid': holdingtid[i]}, {'itemshold':1, '_id':0})
                                            st= str(cur[0]).split("'")
                ...
                #appropriate status(read,write) and transaction table is updated to show the dataitem in tid's held resources
                if count1==0:
                	db.locktable.insert({'itemname':dataitem,'lockstate':'write','lockholdingtid':'','lockwaitingtid':'-w'+x[1]})
                elif count1 > 0:
                	cur = db.locktable.find({'itemname': dataitem}, {'lockwaitingtid':1, '_id':0})
                	st= str(cur[0]).split("'")
                	lockwaitingtid = str(st[3]) + '-' + 'w'+str(x[1])
        ...
        for y in cur:
            print(y)
            o.write('T_ID:'+ str(y['tid']) + '\tTimeStamp:' + str(y['timestamp']) + '\tState:' + str(y['transstate']) +'\n')
        o.write('\n')
        ...
        for y in cur:
            print(y)
            if(str(y['lockholdingtid']) != '' or str(y['lockwaitingtid']) != ''):
                o.write('DataItem:' + str(y['itemname']) + '\tLock_Mode:' + str(y['lockstate']) + '\t\tT_Id_List:' + str(y['lockholdingtid']) + '\tBlockedOperations:' + str(y['lockwaitingtid']) + '\n')
